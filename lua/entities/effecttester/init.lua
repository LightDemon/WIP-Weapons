...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "effecttester" )
		ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:fireeffect()
	...
	if (self.ammos > 0) then
		...
		local effectdata = EffectData()
					...
					effectdata:SetStart(self.Entity:GetPos() + (self.Entity:GetUp() * 20))
					util.Effect( "missiledirt", effectdata ) -- change helecoptermegabomb to the effect name ex athesplode dont forget the quote marks
	else 
		self.armed = false
		self.ammos = 12
